var driveHandler = new function() {
    ...
    // changes appplied to the code regariding photo button: the button will be set to change state.photo to either true or false, it will correspond to photo capturing action specified in web.py
    ...
    var setBindings = function() { //binding the keys to its functionality changes applied 23_08

      $(document).keydown(function(e) {
          if(e.which == 32) { toggleBrake() }  // 'space'  brake
          if(e.which == 82) { toggleRecording() }  // 'r'  toggle recording
          if(e.which == 80) { togglePhoto() }  // 'p'  toggle photo changes applied 13_09
          if(e.which == 87) { throttleUp() }  // 'w'  throttle up
          if(e.which == 83) { throttleDown() } // 's'  slow down
          if(e.which == 65) { angleLeft() } // 'a' turn left
          if(e.which == 68) { angleRight() } // 'd' turn right
          if(e.which == 75) { updateDriveMode('auto') } // 'k' turn on auto mode
          if(e.which == 77) { updateDriveMode('user') } // 'm' turn on manual mode
          if(e.which == 75) { updateDriveMode('auto_angle') } // 'k' turn on auto mode
      ...
      $('#record_button').click(function () {
       ...
       $('#photo_button').click(function () { //28_08
        togglePhoto();
      });
    ...
    var updateUI = function() {
      ...
      // changes apply to unify changes of the names after clicking the buutons 
      ...
      if (state.brakeOn) {
        $('#brake_button')
          .html('START VEHICLE (SPACE)')
          .removeClass('btn-danger')
          .addClass('btn-success').end()
      } else {
        $('#brake_button')
          .html('STOP VEHICLE (SPACE)')
    ...
    var postDrive = function() {
        ...
        //state.photo = false this was just to test how to set the value to true just for a moment, did not work, but stays here as a precaution 
        data = JSON.stringify({ 'angle': state.tele.user.angle,
                                'throttle':state.tele.user.throttle,
                                'drive_mode':state.driveMode,
                                'recording': state.recording,
                                 'photo': state.photo}) //28_08
        console.log(data)
        $.post(driveURL, data)
    ...
    var togglePhoto = function(){ //28-08
