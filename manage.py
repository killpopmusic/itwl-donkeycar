#!/usr/bin/env python3
#below the docstring is specified, it is later called using args in the main function 
"""
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    #below the description of how the parts work together, passing inputs and outputs, this important to understand the process of parts' corealation 
    '''
    ...
    if camera_type == "stereo":
        ...
        V.add(cam, inputs=['photo'], outputs=['cam/image_array'], threaded=threaded) #7-09 applying if condition to add camera again 
        #restart = ['restart']
        
      
    #if restart:
        #V.add(cam, inputs=['photo'], outputs=['cam/image_array', 'restart'], threaded=threaded)
        #print('restarting')
    ...
    V.add(ctr, 
          ...
          outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'photo'], #28_08
          threaded=True)
    ...
    #2nd attempt to make a class "photo" (no way its gonna work)
    ...
    """"class Photo:
        ...
        def __init__(self):
          self.snap = False
          save_path = '/mycar/NDT_photos'
        
        def run(self, photo, image, restart):
          if restart and not self.snap:
               
               print('photo')
               image = ['cam/image_array']
               current_date = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
               filename = f"PHOTO_{current_date}.jpg"
            
            # Assuming 'image' is a NumPy array representing the image
               pil_image = Image.fromarray(image)
               save_path = '/home/piracer/mycar/NDT_photos'
               ...
               self.snap = True
          elif not restart and self.snap:
               print('NO')
                
               self.snap = False
    V.add(Photo(), inputs=['photo', 'cam/image_array', 'restart'])"""
    ...
    if cfg.PUB_CAMERA_IMAGES:
        ...
        pub = TCPServeValue("camera") #TAKING THE PHOTO FROM TCP, THIS MAY ME USEFUL I GUESS
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if type(ctr) is LocalWebController:
        ...
        ctr.print_controls()
            ...
            # # ... (Other parts of the DriveLoop)

    # # Start the drive loop thread
    # drive_loop_thread()
